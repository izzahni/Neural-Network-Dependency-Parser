...
from parsing import minibatch_parse
...
tf.flags.DEFINE_string("optimizer", "adagrad", "optimizer, can be adam or adagrad")
tf.flags.DEFINE_string("output", "output/ud_english-out.txt", "output filename for arcs")
...
class ParserModel(Model):
    ...
    def add_placeholders(self):
        ...
        """
        self.word_id_placeholder = tf.compat.v1.placeholder(
            tf.int32,
            shape=(None, self.config.n_word_features)
        )
        self.tag_id_placeholder = tf.compat.v1.placeholder(
            tf.int32,
            shape=(None, self.config.n_tag_features)
        )
        self.deprel_id_placeholder = tf.compat.v1.placeholder(
            tf.int32,
            shape=(None, self.config.n_deprel_features)
        )
        self.class_placeholder = tf.compat.v1.placeholder(
            tf.float32,
    ...
    def add_prediction_op(self):
        ...
        if FLAGS.hidden:
            ...
            for i in range(FLAGS.hidden):
                ...
                b[i] = tf.Variable(tf.random.normal([self.config.hidden_size]))
    ...
    def add_training_op(self, loss):
        ...
        if FLAGS.optimizer == 'adagrad':
            optimizer = tf.compat.v1.train.AdagradOptimizer(self.config.lr)
        else:
